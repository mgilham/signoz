...
type prepareTracesQueryFunc func(ctx context.Context, start, end int64, panelType v3.PanelType, bq *v3.BuilderQuery, options v3.QBOptions) (string, error)
type prepareLogsQueryFunc func(ctx context.Context, start, end int64, queryType v3.QueryType, panelType v3.PanelType, bq *v3.BuilderQuery, options v3.QBOptions) (string, error)
...
func (qb *QueryBuilder) PrepareLiveTailQuery(ctx context.Context, params *v3.QueryRangeParamsV3) (string, error) {
	var queryStr string
	var err error
	...
	if compositeQuery != nil {
		...
		for queryName, query := range compositeQuery.BuilderQueries {
			if query.Expression == queryName {
				queryStr, err = qb.options.BuildLogQuery(ctx, params.Start, params.End, compositeQuery.QueryType, compositeQuery.PanelType, query, v3.QBOptions{IsLivetailQuery: true})
				if err != nil {
					return "", err
				}
			}
...
func (qb *QueryBuilder) PrepareQueries(ctx context.Context, params *v3.QueryRangeParamsV3) (map[string]string, error) {
			...
			if query.Expression == queryName {
				...
				case v3.DataSourceTraces:
					...
					if compositeQuery.PanelType == v3.PanelTypeGraph && query.Limit > 0 && len(query.GroupBy) > 0 {
						limitQuery, err := qb.options.BuildTraceQuery(ctx, start, end, compositeQuery.PanelType, query,
							v3.QBOptions{GraphLimitQtype: constants.FirstQueryGraphLimit, PreferRPM: PreferRPMFeatureEnabled})
						if err != nil {
							return nil, err
						}
						placeholderQuery, err := qb.options.BuildTraceQuery(ctx, start, end, compositeQuery.PanelType,
						...
						queryString, err := qb.options.BuildTraceQuery(ctx, start, end, compositeQuery.PanelType,
				...
				case v3.DataSourceLogs:
					...
					if compositeQuery.PanelType == v3.PanelTypeGraph && query.Limit > 0 && len(query.GroupBy) > 0 {
						limitQuery, err := qb.options.BuildLogQuery(ctx, start, end, compositeQuery.QueryType, compositeQuery.PanelType, query, v3.QBOptions{GraphLimitQtype: constants.FirstQueryGraphLimit, PreferRPM: PreferRPMFeatureEnabled})
						if err != nil {
							return nil, err
						}
						placeholderQuery, err := qb.options.BuildLogQuery(ctx, start, end, compositeQuery.QueryType, compositeQuery.PanelType, query, v3.QBOptions{GraphLimitQtype: constants.SecondQueryGraphLimit, PreferRPM: PreferRPMFeatureEnabled})
						if err != nil {
							return nil, err
						}
						query := fmt.Sprintf(placeholderQuery, limitQuery)
						...
						queryString, err := qb.options.BuildLogQuery(ctx, start, end, compositeQuery.QueryType, compositeQuery.PanelType, query, v3.QBOptions{PreferRPM: PreferRPMFeatureEnabled, GraphLimitQtype: ""})
						if err != nil {
							return nil, err
						}
						queries[queryName] = queryString
